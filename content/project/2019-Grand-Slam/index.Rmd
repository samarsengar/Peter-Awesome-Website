---
title: Serving Efficiency at Grand Slams
author: Peter Tea
date: '2019-12-26'
categories:
  - Tennis
...
A <- to_plot_dat %>%
  ...
  select(Player_Name, Matches, SvGmsW_Prop, BPW_Prop)

knitr::kable(A[1:10,], caption = "Grand Slam data")
```

During the grand slams Nadal, Djokovic and Federer are ranked 1, 2 and 3 respectively in terms of total matches played. The other two columns we calculated represents the proportion of service games defended (**SvGmsW_Prop**), and the proportion of opponent serves broken (**BPW_Prop**).


### Comparing the big 3 to other ATP players via Bubble plots
Using the two metrics (proportion of service games defended and the proportion of opponent serves broken) we can map out the deffensive and offensive prowess of the Big 3 compared to the abilities of the other ATP players. We place these two metrics on the x- and y-axis respectively. Additionally, to represent the success of the ATP players, we add an extra dimension that represents number of matches won by each player. The larger the *bubble*, the more grand slam matches a player has played (which is directly related to the number wins he has during the grand slams).

```{r pressure2,echo=FALSE,  warning=FALSE}

p <- ggplot(to_plot_dat[-c(1,48,64),], aes(x=SvGmsW_Prop*100, y=BPW_Prop*100, size = Matches, label = Player_Name)) + 
  ...
  geom_point(data=to_plot_dat[1, ], aes(x=SvGmsW_Prop*100, y=BPW_Prop*100, size = Matches),
             colour="blue", alpha = 0.7) +
  
  #Colour Federer in Green
  geom_point(data=to_plot_dat[48, ], aes(x=SvGmsW_Prop*100, y=BPW_Prop*100, size = Matches),
             colour="forestgreen", alpha = 0.7) +
  
  #Colour Rafa in Red
  geom_point(data=to_plot_dat[64, ], aes(x=SvGmsW_Prop*100, y=BPW_Prop*100, size = Matches),
...
For those who adore interactive plots, I've produced the slame plot using the R package ``gplotly``.
